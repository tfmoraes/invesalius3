...
from aesara.sandbox.rng_mrg import MRG_RandomStream as RandomStreams
from aesara.tensor.signal import pool
from aesara.printing import Print
from aesara.ifelse import ifelse
try:
    import aesara.sparse as th_sparse_module
except ImportError:
    th_sparse_module = None
try:
    from aesara.tensor.nnet import softsign as T_softsign
except ImportError:
    from aesara.sandbox.softsign import softsign as T_softsign
...
def _assert_sparse_module():
    if not th_sparse_module:
        raise ImportError("Failed to import aesara.sparse\n"
                          "You probably need to pip install nose-parameterized")


def is_sparse(tensor):
    return th_sparse_module and isinstance(tensor.type, th_sparse_module.type.SparseType)
...
def variable(value, dtype=None, name=None, constraint=None):
    ...
    if hasattr(value, 'tocoo'):
        ...
        if isinstance(value, (aesara.tensor.TensorVariable,
                              aesara.tensor.sharedvar.TensorSharedVariable,
                              aesara.tensor.TensorConstant)):
            # Support for RandomStreams().normal(), .uniform().
            value = value.eval()
        value = np.asarray(value, dtype=dtype)
        variable = aesara.shared(value=value,
...
def is_placeholder(x):
    ...
    # Returns
        Boolean.
    """
    return hasattr(x, '_aesara_placeholder') and x._aesara_placeholder
...
def shape(x):
    ...
    aesara backend (aesara tensor type) and TF backend (TF TensorShape).
    """
    return x.shape
...
def count_params(x):
    ...
    """
    # We don't want those compilation to show up in aesara profiler.
    f = aesara.function([], x.shape, profile=False)
    return np.prod(f())
...
def batch_dot(x, y, axes=None):
    ...
    # workaround because aesara doesn't accept axes
...
def mean(x, axis=None, keepdims=False):
    ...
    """
    dtype = None
    # bool is available since aesara v0.9dev
...
def logsumexp(x, axis=None, keepdims=False):
    ...
    # Returns
        The reduced tensor.
    """
    # aesara has a built-in optimization for logsumexp (see https://github.com/aesara/aesara/pull/4736)
...
def normalize_batch_in_training(x, gamma, beta,
    ...
    """
    # TODO remove this if statement when aesara without
...
# TODO remove this function when aesara without
# T.nnet.bn.batch_normalization_train is deprecated
def _old_normalize_batch_in_training(x, gamma, beta, reduction_axes,
    ...
    dev = aesara.config.device
    use_cudnn = ndim(x) < 5 and reduction_axes == [0, 2, 3] and (dev.startswith('cuda') or dev.startswith('gpu'))
    if use_cudnn:
        broadcast_beta = beta.dimshuffle('x', 0, 'x', 'x')
        broadcast_gamma = gamma.dimshuffle('x', 0, 'x', 'x')
        try:
            normed, mean, stdinv = aesara.sandbox.cuda.dnn.dnn_batch_normalization_train(
                x, broadcast_gamma, broadcast_beta, 'spatial', epsilon)
            normed = aesara.tensor.as_tensor_variable(normed)
            mean = aesara.tensor.as_tensor_variable(mean)
            stdinv = aesara.tensor.as_tensor_variable(stdinv)
            var = T.inv(stdinv ** 2)
...
def _old_batch_normalization(x, mean, var, beta, gamma,
    ...
    if mean.ndim == 1 and x.ndim > 1:
        ...
        # aesara expects the parameters to always have x.ndim dimensions.
    ...
    if use_cudnn:
        try:
            ...
            if axis != 1:
                ...
                result = aesara.sandbox.cuda.dnn.dnn_batch_normalization_test(
            ...
            return aesara.tensor.as_tensor_variable(result)
        except AttributeError:
            pass
        except ValueError:
            pass
    ...
    if interpolation == 'nearest':
        ...
        if not (height_factor == width_factor == 2):
            raise NotImplementedError(
                ...
                'is not available when using the aesara backend.')
...
def arange(start, stop=None, step=1, dtype='int32'):
    ...
    aesara's arange: if only one argument is provided,
...
def temporal_padding(x, padding=(1, 1)):
    ...
    Apologies for the inane API, but aesara makes this
    really hard.
    """
...
class Function(object):
    ...
    def __init__(self, inputs, outputs, updates=[], name=None, **kwargs):
        ...
        self.function = aesara.function(inputs, outputs, updates=updates,
...
def function(inputs, outputs, updates=[], **kwargs):
    if len(kwargs) > 0:
        for key in kwargs.keys():
            if not has_arg(aesara.function, key, True):
                msg = 'Invalid argument "%s" passed to K.function with aesara backend' % key
...
def stop_gradient(variables):
    ...
    if isinstance(variables, (list, tuple)):
        return map(aesara.gradient.disconnected_grad, variables)
    else:
        return aesara.gradient.disconnected_grad(variables)
...
def rnn(step_function, inputs, initial_states,
    ...
    if mask is not None:
        ...
        if unroll:
            ...
            # aesara gets confused by broadcasting patterns in the scan op
            ...
            # deal with aesara API inconsistency
        ...
        if unroll:
            ...
            # aesara likes to make shape==1 dimensions
...
def _assert_has_capability(module, func):
    if not hasattr(module, func):
        raise EnvironmentError(
            ...
            'aesara is out of date. '
            'Install the latest version with:\n'
            'pip install git+git://github.com/aesara/aesara.git '
            '--upgrade --no-deps')
...
def sigmoid(x):
    return T.nnet.sigm.sigmoid(x)


def hard_sigmoid(x):
    return T.nnet.sigm.hard_sigmoid(x)
...
def in_top_k(predictions, targets, k):
    ...
    if k < 1:
        # dtype='bool' is only available since aesara 0.9.0
        try:
...
def _preprocess_conv2d_kernel(kernel, data_format):
    ...
    # aesara expects `(depth, input_depth, rows, cols)`.
...
def _preprocess_conv2d_depthwise_kernel(kernel, kernel_shape, data_format):
    ...
    # on the format `(rows, cols, input_depth, depth)`,
    # independently of `data_format`.
    # aesara expects `(input_depth * depth, 1, rows, cols)` for depthwise convolution.
    kernel = kernel[::-1, ::-1, :, :]
...
def _preprocess_conv3d_kernel(kernel, data_format):
    ...
    # independently of `data_format`.
    # aesara expects `(depth, input_depth, space)`.
...
def _preprocess_conv2d_image_shape(image_shape, data_format):
    # aesara might not accept long type
...
def conv2d(x, kernel, strides=(1, 1), padding='valid',
    ...
    # Arguments
        ...
        data_format: "channels_last" or "channels_first".
            Whether to use aesara or TensorFlow data format
...
def conv2d_transpose(x, kernel, output_shape, strides=(1, 1),
    ...
    if padding == 'same' and kernel_shape[0] % 2 == 0:
        raise ValueError('In `Conv2DTranspose`, with padding mode `same`, '
                         'even kernel sizes are not supported with aesara. '
...
def ctc_path_probs(predict, Y, alpha=1e-4):
    ...
    [f_active, log_f_probs, b_active, log_b_probs], _ = aesara.scan(
...
def map_fn(fn, elems, name=None, dtype=None):
    ...
    # Returns
        ...
        fn
    """
    return aesara.map(fn, elems, name=name)[0]
...
def foldl(fn, elems, initializer=None, name=None):
    ...
    # We need to change the order of the arguments because aesara accepts x as
    # first parameter and accumulator as second
    return aesara.foldl(lambda x, acc: fn(acc, x),
...
def foldr(fn, elems, initializer=None, name=None):
    ...
    # We need to change the order of the arguments because aesara accepts x as
    # first parameter and accumulator as second
    return aesara.foldr(lambda x, acc: fn(acc, x),
                        elems, initializer, name=name)[0]
