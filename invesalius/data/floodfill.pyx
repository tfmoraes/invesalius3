...
def floodfill(np.ndarray[DTYPE_t, ndim=3] data, int i, int j, int k, int v, int fill, np.ndarray[DTYPE_t, ndim=3] out):
...
def floodfill_threshold(np.ndarray[DTYPE16_t, ndim=3] data, int i, int j, int k, int t0, int t1, int fill, np.ndarray[DTYPE_t, ndim=3] out):
...
@cython.wraparound(False)
@cython.nonecheck(False)
def floodfill_auto_threshold(np.ndarray[DTYPE16_t, ndim=3] data, int i, int j, int k, float p, int fill, np.ndarray[DTYPE_t, ndim=3] out):
    ...
    stack = deque()
    t0 = <int>floor(data[k, j, i] * (1 - p))
    t1 = <int>floor(data[k, j, i] * (1 + p))
    #if data[k, j, i] >= t0 and data[k, j, i] <= t1:
    stack.append((i, j, k))
    out[k, j, i] = fill
